...
public class CardEdit extends JPanel {
	final CardEdit me =  this;
	 ...
	 */
	public CardEdit(final Project proj, final MainFrame p, Card car) throws InterruptedException, SQLException {
		setLayout(new BorderLayout(0, 0));
		card = car;
		JLabel label = new JLabel("Амбулаторная карта " + car.getPacient().getSurname() + " "+ car.getPacient().getName()+ " "+car.getPacient().getOtchestvo());
		...
		JButton btnNewButton_3 = new JButton("Просмотр листов осмотра");
		btnNewButton_3.addActionListener(new ActionListener() {
			public void actionPerformed(ActionEvent e) {
				Lists li;
				try {
					li= new Lists(proj, p,card);
					li.setVisible(true);
					me.revalidate();
					p.contentPane.remove(me);
					p.contentPane.add(li,BorderLayout.CENTER);
				} catch (InterruptedException e1) {
					// TODO Auto-generated catch block
					e1.printStackTrace();
				} catch (SQLException e1) {
